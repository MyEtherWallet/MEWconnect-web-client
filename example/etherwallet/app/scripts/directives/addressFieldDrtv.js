    ...
    link: function(scope, element, attrs) {
      ...
      var placeholder =
        attrs.placeholder == undefined
          ? 'mewtopia.eth or 0xDECAF9CD2367cdbb726E904cD6397eDFcAe6068D'
          : attrs.placeholder;
      var labelTranslated =
      ...
      var checkDarkList = function(value) {
        for (let i = 0; i < Darklist.length; i++) {
          if (
            value.length > 0 &&
            value.toLowerCase() === Darklist[i].address.toLowerCase()
          ) {
            scope.phishing.msg =
              Darklist[i].comment !== ''
                ? `${globalFuncs.phishingWarning[0] + Darklist[i].comment}`
                : `${globalFuncs.phishingWarning[1]}`;
      ...
      scope.$watch('addressDrtv.ensAddressField', function() {
        ...
        if (Validator.isValidAddress(scope.addressDrtv.ensAddressField)) {
          ...
          if (scope.addressDrtv.ensAddressField !== '') {
            scope.phishing.msg =
              'Invalid address or ENS. Please make sure that the address or ENS you put in is valid';
            scope.phishing.error = true;
          } else {
            scope.addressDrtv.showDerivedAddress = false;
            scope.phishing.msg = '';
