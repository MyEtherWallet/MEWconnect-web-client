...
var contractsCtrl = function($scope, $sce, walletService) {
  ...
  $scope.contract = {
    address:
      ...
      $scope.Validator.isValidAddress(globalFuncs.urlGet('address'))
  ...
  $scope.generateTx = function() {
    try {
      if ($scope.wallet == null) throw globalFuncs.errorMsgs[3];
        ...
        !globalFuncs.isNumeric($scope.tx.gasLimit) ||
        parseFloat($scope.tx.gasLimit) <= 0
      )
  ...
  $scope.readFromContract = function() {
    ajaxReq.getEthCall(
      { to: $scope.contract.address, data: $scope.getTxData() },
      function(data) {
        if (!data.error) {
          var curFunc =
  ...
  $scope.initContract = function() {
    try {
      ...
      else if (!$scope.Validator.isJSON($scope.contract.abi))
