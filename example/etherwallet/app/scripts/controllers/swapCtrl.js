...
var swapCtrl = function($scope, $sce, walletService) {
  ...
  $scope.resetErrorShownStates = function() {
    $scope.showedMinMaxError = false;
    if ($scope.showedKyberPairAvailableError)
  ...
  $scope.verifyBityMinMaxValues = function() {
    ...
    var verify = function() {
      if (!$scope.bity.priceLoaded) return errors.priceNotLoaded;
        ...
        $scope.swapOrder.toVal < bity.min ||
        $scope.swapOrder.fromVal < bity.min
      )
        return errors.lessThanMin;
      else if (
        ($scope.swapOrder.toCoin == 'BTC' &&
          $scope.swapOrder.toVal > bity.max) ||
        ($scope.swapOrder.fromCoin == 'BTC' &&
          $scope.swapOrder.fromVal > bity.max)
      )
    ...
    if (vResult == errors.noErrors) return true;
      ...
      vResult == errors.lessThanMin ||
      vResult == errors.greaterThanMax
    ) {
  ...
  $scope.updateBityEstimate = function(isFrom) {
    if (isFrom)
      $scope.swapOrder.toVal = parseFloat(
        (
          $scope.bity.curRate[
            $scope.swapOrder.fromCoin + $scope.swapOrder.toCoin
          ] * $scope.swapOrder.fromVal
        ).toFixed(bity.decimals)
      );
    else
      $scope.swapOrder.fromVal = parseFloat(
        (
          $scope.swapOrder.toVal /
          $scope.bity.curRate[
            $scope.swapOrder.fromCoin + $scope.swapOrder.toCoin
  ...
  $scope.setKyberOrderCoin = async function(isFrom) {
    ...
    if (_availableResult) {
      $scope.swapOrder.swapRate =
        $scope.kyber.kyberRates[
          kyber.toPairKey($scope.swapOrder.fromCoin, $scope.swapOrder.toCoin)
        ];
      $scope.swapOrder.swapPair =
        $scope.swapOrder.fromCoin + '/' + $scope.swapOrder.toCoin;
  ...
  $scope.isKyberPairAvailable = async function() {
    try {
      let _returnedRate = await $scope.kyber.setKyberRate(
  ...
  $scope.kyberSwapRateDisplay = function() {
    if ($scope.checkIfKyber())
      $scope.kyberSwapRateDisplayValue =
        $scope.kyber.kyberRates[
          kyber.toPairKey($scope.swapOrder.fromCoin, $scope.swapOrder.toCoin)
        ];
  };
  ...
  $scope.checkIfKyber = function() {
    $scope.isKyberRateSwap =
      ($scope.availableTokens.indexOf($scope.swapOrder.fromCoin) >= 0 ||
        $scope.swapOrder.fromCoin == 'ETH') &&
      ($scope.availableTokens.indexOf($scope.swapOrder.toCoin) >= 0 ||
        $scope.swapOrder.toCoin == 'ETH');
  ...
  $scope.verifyKyberMinMaxValues = function() {
    ...
    let verify = function() {
      if (!$scope.kyber.priceLoaded) return errors.priceNotLoaded;
        ...
        return errors.pairNotAvailable;
      else if (
        $scope.kyber.kyberRates[
          kyber.toPairKey($scope.swapOrder.fromCoin, $scope.swapOrder.toCoin)
        ] == '0'
  ...
  $scope.updateKyberEstimate = function(isFrom) {
    try {
      if (
        /,/.test($scope.swapOrder.fromVal) ||
        /,/.test($scope.swapOrder.toVal)
      ) {
      ...
      if (isFrom) {
        let toDecimalsToUse =
          $scope.kyber.tokenDetails[$scope.swapOrder.toCoin].decimals <
          bity.decimals
            ? $scope.kyber.tokenDetails[$scope.swapOrder.toCoin].decimals
            : bity.decimals;
        let _toVal =
        ...
        let fromDecimalsToUse =
          $scope.kyber.tokenDetails[$scope.swapOrder.fromCoin].decimals <
          bity.decimals
            ? $scope.kyber.tokenDetails[$scope.swapOrder.fromCoin].decimals
            : bity.decimals;
        let _fromVal =
        ...
        $scope.swapOrder.fromVal = parseFloat(
          _fromVal.toFixed(fromDecimalsToUse)
        );
      }
  ...
  $scope.updateFinalKyberEstimate = function(isFrom, callback) {
    try {
      $scope.kyber.getExpectedRate(
        $scope.swapOrder.fromCoin,
        $scope.swapOrder.toCoin,
        $scope.swapOrder.fromVal,
        function(_rates) {
          let finalRate = $scope.kyber.convertToTokenBase(
            _rates.data.slippageRate,
            'ETH'
          );
          ...
          if (isFrom) {
            ...
            $scope.swapOrder.fromVal = parseFloat(
              _fromVal.toFixed(bity.decimals)
            );
          }
          if ($scope.swapOrder.fromVal == Infinity) {
            $scope.swapOrder.fromVal = '';
  ...
  var checkForPriorTokenApproval = function(_address, callback) {
    ...
    $scope.kyber.allowance($scope.kyberSwapOrder.fromCoin, _address, function(
      data
    ) {
      if (data.error) $scope.notifier.danger(data.msg);
      else {
        // data = data.data;
        if (data.data > 0) {
          let allocationNeeded = $scope.kyber.convertToTokenBase(
            $scope.kyberSwapOrder.toVal,
            $scope.kyberSwapOrder.toCoin
          );
          $scope.kyberTransaction.currentTokenApprovalValue = $scope.kyber.convertToTokenBase(
            data.data,
            $scope.kyberSwapOrder.fromCoin
          );
          if (
            $scope.kyberTransaction.currentTokenApprovalValue > allocationNeeded
          ) {
            callback('bypass');
          } else {
            $scope.kyberTransaction.currentTokenApprovalValue = $scope.kyber.convertToTokenBase(
    ...
    try {
      if ($scope.checkTokenBalance) {
        ...
        checkUserCap(_userAddress, function(result) {
          if (!result.error) {
            $scope.kyber.getBalance(
              $scope.kyberSwapOrder.fromCoin,
              _userAddress,
              function(_result) {
                let userTokenBalance = new BigNumber(
                  $scope.kyber.convertToTokenBase(
                    _result.data,
                    $scope.kyberSwapOrder.fromCoin
                  )
                );
                let enoughTokens = userTokenBalance.gte(
                  $scope.kyberSwapOrder.fromVal
                );
                if (enoughTokens) {
                  checkForPriorTokenApproval(
                    $scope.walletKyber.getAddressString(),
                    _data => {
  ...
  $scope.validKyberPair = function() {
    let hasEth =
      $scope.swapOrder.fromCoin == 'ETH' || $scope.swapOrder.toCoin == 'ETH';
    let notBTC =
      $scope.swapOrder.fromCoin !== 'BTC' || $scope.swapOrder.toCoin !== 'BTC';
    let notREP =
      $scope.swapOrder.fromCoin !== 'REP' || $scope.swapOrder.toCoin !== 'REP';
  ...
  $scope.startKyber = function() {
    ...
    try {
      ...
      $scope.kyberModal = new Modal(
        document.getElementById('kyberTransaction')
      );
  ...
  $scope.openKyberEthOrder = function() {
    try {
      ...
      $scope.tx = $scope.buildTransactionObject(
        $scope.kyber.getTradeData(
          $scope.kyberSwapOrder,
          $scope.kyberSwapOrder.finalRate
        ),
        $scope.kyber.getKyberNetworkAddress(),
        $scope.kyberSwapOrder.fromVal
      );
  ...
  $scope.checkForResetApproveTokenKyber = function() {
    try {
      if ($scope.kyberTransaction.tokenNeedsReset) {
        $scope.tx = $scope.buildTransactionObject(
          $scope.kyber.approveKyber($scope.kyberSwapOrder.fromCoin, 0),
          $scope.kyber.getTokenAddress($scope.kyberSwapOrder.fromCoin)
        );
        var txData = uiFuncs.getTxData($scope);
        txData.nonce = txData.gasPrice = null;
        return $scope.generateKyberTransaction(
          txData,
          $scope.kyberStatus.token.resetApprove
        );
      } else {
  ...
  $scope.approveTokenKyber = function(nonce, gasPrice) {
    ...
    try {
      ...
      return $scope.generateKyberTransaction(
        txData,
        $scope.kyberStatus.token.prepareApprove
      );
    } catch (e) {
      console.error(e);
    }
  ...
  $scope.openKyberTokenOrder = function(nonce, gasPrice) {
    try {
      ...
      return $scope.generateKyberTransaction(
        txData,
        $scope.kyberStatus.token.prepare
      );
    } catch (e) {
      console.error(e);
    }
  ...
  */
    ...
    try {
      uiFuncs.sendTx(signedTx, function(resp) {
        if (!resp.isError) {
          ...
          switch ($scope.kyberOrderResult.progress.status) {
            case 'RESET_TOKEN_APPROVAL':
              $scope.kyberTransaction.tokenResetTxHash = notCustomNode
                ? resp.data
                : '';
              $scope.kyberTransaction.tokenResetTxLink = $scope.ajaxReq.blockExplorerTX.replace(
                '[[txHash]]',
                resp.data
              );
              ...
              $scope.checkForTokenResetKyber(
                $scope.walletKyber.getAddressString()
              );
              break;
            case 'TOKENS_APPROVED':
              $scope.kyberTransaction.tokenTxHash = notCustomNode
                ? resp.data
                : '';
              $scope.kyberTransaction.tokenTxLink = $scope.ajaxReq.blockExplorerTX.replace(
                '[[txHash]]',
                resp.data
              );
              $scope.displayTxHashPopup(
                resp,
                $scope.kyberTransaction.tokenTxLink
              );
              $scope.kyberOrderResult.progress.bar = getProgressBarArr(5, 5);
              break;
            case 'AWAITING_TOKEN_APPROVAL':
              $scope.kyberTransaction.tokenApproveTxHash = notCustomNode
                ? resp.data
                : '';
              $scope.kyberTransaction.tokenApproveTxLink = $scope.ajaxReq.blockExplorerTX.replace(
                '[[txHash]]',
                resp.data
              );
              ...
              break;
            case 'SEND_ETH':
              $scope.kyberTransaction.ethTxHash = notCustomNode
                ? resp.data
                : '';
              $scope.kyberTransaction.ethTxLink = $scope.ajaxReq.blockExplorerTX.replace(
                '[[txHash]]',
                resp.data
              );
              $scope.kyberOrderResult.progress.bar = getProgressBarArr(5, 5);
              $scope.displayTxHashPopup(
                resp,
                $scope.kyberTransaction.ethTxLink
              );
              break;
            default:
              break;
  ...
  $scope.buildTransactionObject = function(data, to, value, gasPrice) {
    let gasLimit;
    if (
      ($scope.kyberSwapOrder.fromCoin === 'DGX' ||
        $scope.kyberSwapOrder.toCoin === 'DGX') &&
  ...
  $scope.buildKyberOrderResultProgress = function() {
    return {
      bar: getProgressBarArr(1, 5),
      weiValue: $scope.kyber.convertToTokenBase(
        $scope.kyberSwapOrder.fromVal,
        'ETH'
      ),
      pendingStatusReq: false,
  ...
  $scope.displayTxHashPopup = function(resp, txHashInject) {
    var emailLink =
      '<a class="strong" href="#" target="_blank" rel="noopener noreferrer">Confused? Email Us.</a>'; // email link
    var bExStr =
      $scope.ajaxReq.type != nodes.nodeTypes.Custom
        ? `<a class="strong" href="${txHashInject}" target="_blank" rel="noopener"> View your transaction </a>`
        : '';
    $scope.sendTxStatus +=
      globalFuncs.successMsgs[2] +
      '<p>' +
  ...
  $scope.displayKyberErrorMessage = function(type, params) {
    switch (type) {
      case 'userCap':
        let message;
        if (params.isFrom) {
          message = `The Supplied ETH value of ${
            params.originalValue
          } is Greater than your current Kyber Network Maximum of ${
            params.userCap
          }. `;
        } else {
          message = `The Resulting ETH value of ${
            params.originalValue
          } is Greater than your current Kyber Network Maximum of ${
            params.userCap
          }. `;
      ...
      case 'zeroRate':
        uiFuncs.notifier.danger(
          `Kyber Network does not support a trade of value ${
            $scope.swapOrder.fromVal
          } ${$scope.swapOrder.fromCoin} to ${$scope.swapOrder.toCoin}`,
          5000
        );
        break;
      case 'tokenBalance':
        uiFuncs.notifier.danger(
          ` The Supplied ${$scope.swapOrder.fromCoin} value of ${
            $scope.swapOrder.fromVal
          } is Greater than your current ${
            $scope.swapOrder.fromCoin
          } Balance of ${params.userTokenBalance.toString()} ${
            $scope.swapOrder.fromCoin
          }. `,
          5000
        );
      ...
      case 'unAvailable':
        uiFuncs.notifier.danger(
          kyber.kyberUnavailablePhrasing(
            $scope.swapOrder.fromCoin,
            $scope.swapOrder.toCoin
          ),
          3500
      ...
      case 'technicalError':
        uiFuncs.notifier.danger(
          "A technical error occurred. No transaction sent. If this error continues please contact support with error code: 'BYPASS'."
        );
        break;
    }
  };
  ...
  $scope.parseKyberSignedTx = function(signedTx) {
    ...
    if (
      !$scope.kyberEthToToken &&
      !$scope.kyberTransaction.bypassTokenApprove
    ) {
      $scope.parsedKyberTokenTx = $scope.parseKyberSignedTransaction(
        $scope.kyberTransaction.tokenApproveTx.signedTx
      );
      if ($scope.kyberTransaction.tokenNeedsReset) {
        $scope.parsedKyberResetTokenTx = $scope.parseKyberSignedTransaction(
          $scope.kyberTransaction.tokenResetTx.signedTx
        );

        // Calculate Combined Values (three transaction case)
        $scope.parsedTx = {};
        $scope.parsedTx.totalTxFee = {};
        $scope.parsedTx.totalTxFee.wei = new BigNumber(
          $scope.parsedKyberTx.txFee.wei
        )
          .plus(new BigNumber($scope.parsedKyberTokenTx.txFee.wei))
          .plus(new BigNumber($scope.parsedKyberResetTokenTx.txFee.wei));
        $scope.parsedTx.totalTxFee.gwei = new BigNumber(
          $scope.parsedKyberTx.txFee.gwei
        )
          .plus(new BigNumber($scope.parsedKyberTokenTx.txFee.gwei))
          .plus(new BigNumber($scope.parsedKyberResetTokenTx.txFee.gwei));
        $scope.parsedTx.totalTxFee.eth = new BigNumber(
          $scope.parsedKyberTx.txFee.eth
        )
          .plus(new BigNumber($scope.parsedKyberTokenTx.txFee.eth))
          .plus(new BigNumber($scope.parsedKyberResetTokenTx.txFee.eth));
        $scope.parsedTx.avgGasPrice = {};
        $scope.parsedTx.avgGasPrice.wei = new BigNumber(
          $scope.parsedKyberTx.gasPrice.wei
        )
          .plus(new BigNumber($scope.parsedKyberTokenTx.gasPrice.wei))
          .plus(new BigNumber($scope.parsedKyberResetTokenTx.gasPrice.wei))
          .div(3);
        $scope.parsedTx.avgGasPrice.gwei = new BigNumber(
          $scope.parsedKyberTx.gasPrice.gwei
        )
          .plus(new BigNumber($scope.parsedKyberTokenTx.gasPrice.gwei))
          .plus(new BigNumber($scope.parsedKyberResetTokenTx.gasPrice.gwei))
          .div(3);
        $scope.parsedTx.avgGasPrice.eth = new BigNumber(
          $scope.parsedKyberTx.gasPrice.eth
        )
          .plus(new BigNumber($scope.parsedKyberTokenTx.gasPrice.eth))
          .plus(new BigNumber($scope.parsedKyberResetTokenTx.gasPrice.eth))
          .div(3);
        $scope.parsedTx.totalGasLimit = new BigNumber(
          $scope.parsedKyberTokenTx.gasLimit
        )
          .plus(new BigNumber($scope.parsedKyberTx.gasLimit))
          .plus(new BigNumber($scope.parsedKyberResetTokenTx.gasLimit));
      } else {
  ...
  $scope.parseKyberSignedTransaction = function(signedTx) {
    ...
    parsedKyberTx.from = isJSON
      ? $scope.walletKyber.getChecksumAddressString()
      : ethFuncs.sanitizeHex(
          ethUtil.toChecksumAddress(txData.from.toString('hex'))
        );
    parsedKyberTx.to = ethFuncs.sanitizeHex(
      ethUtil.toChecksumAddress(txData.to.toString('hex'))
    );
    parsedKyberTx.value =
      txData.value == '0x' || txData.value == '' || txData.value == null
        ? '0'
        : etherUnits.toEther(
            new BigNumber(
              ethFuncs.sanitizeHex(txData.value.toString('hex'))
            ).toString(),
            'wei'
          );
    parsedKyberTx.gasLimit = new BigNumber(
      ethFuncs.sanitizeHex(txData.gasLimit.toString('hex'))
    ).toString();
    parsedKyberTx.gasPrice.wei = new BigNumber(
      ethFuncs.sanitizeHex(txData.gasPrice.toString('hex'))
    ).toString();
    parsedKyberTx.gasPrice.gwei = new BigNumber(
      ethFuncs.sanitizeHex(txData.gasPrice.toString('hex'))
    )
      .div(etherUnits.getValueOfUnit('gwei'))
      .toString();
    parsedKyberTx.gasPrice.eth = etherUnits.toEther(
      new BigNumber(
        ethFuncs.sanitizeHex(txData.gasPrice.toString('hex'))
      ).toString(),
      'wei'
    );
    parsedKyberTx.txFee.wei = new BigNumber(
      parseInt(parsedKyberTx.gasLimit)
    ).times(new BigNumber(parseInt(parsedKyberTx.gasPrice.wei)));
    parsedKyberTx.txFee.gwei = new BigNumber(parsedKyberTx.txFee.wei)
      .div(etherUnits.getValueOfUnit('gwei'))
      .toString();
    parsedKyberTx.txFee.eth = etherUnits
      .toEther(parseInt(parsedKyberTx.txFee.wei), 'wei')
      .toString();
    parsedKyberTx.nonce =
      txData.nonce == '0x' || txData.nonce == '' || txData.nonce == null
        ? '0'
        : new BigNumber(
            ethFuncs.sanitizeHex(txData.nonce.toString('hex'))
          ).toString();
    parsedKyberTx.data =
      txData.data == '0x' || txData.data == '' || txData.data == null
        ? '(none)'
        : ethFuncs.sanitizeHex(txData.data.toString('hex'));
