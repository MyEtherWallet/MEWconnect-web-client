...
var tabsCtrl = function($scope, globalService, $translate, $sce) {
  ...
  $scope.changeNode = function(key) {
    ...
    if (nodes.domainsaleNodeTypes.indexOf($scope.curNode.type) == -1)
      $scope.tabNames.domainsale.cx = $scope.tabNames.domainsale.mew = false;
      ...
      if (data.error) {
        ...
        $scope.notifier.info(
          globalFuncs.successMsgs[5] +
            '<br /> URL: <strong>' +
            globalFuncs.stripTags(window.location.href) +
  ...
  globalFuncs.MEWconnectStatus.registerNewTabOpener(
    $scope.newTabOpenedMEWconnect.bind(this)
  );
  globalFuncs.MEWconnectStatus.register(
    $scope.displayMEWconnectState.bind(this)
  );
};
