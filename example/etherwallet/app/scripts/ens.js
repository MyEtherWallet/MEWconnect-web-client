...
var ens = function() {
  ...
  this.subDomainABI = {};
  for (var i in subDomainInterface)
    this.subDomainABI[subDomainInterface[i].name] = subDomainInterface[i];
  switch (ajaxReq.type) {
...
ens.prototype.getSubDomainBuyData = function(domain, subdomain, sender) {
  ...
  if (domain.version == '0.9')
    dataString = _this.getDataString(funcABI, [
      _this.getSHA3(domain.name),
      subdomain,
      sender,
      _this.curRegistry.public.resolver,
...
ens.prototype.checkDomain = async function(domain, subdomain) {
  ...
  var dataString = _this.getDataString(funcABI, [
    _this.getSHA3(domain.name),
    subdomain
  ]);
  return new Promise((resolve, reject) => {
