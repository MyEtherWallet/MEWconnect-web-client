...
globalFuncs.gethErrors = {
  ...
  '(geth-02) Gas price too low for acceptance\\. Try raising the gas price to 21 GWEI via the dropdown in top-right\\.':
    'GETH_Cheap',
  '(geth-01) Insufficient balance\\. Your gas limit * gas price + amount to send exceeds your current balance\\. Send more ETH to your account or use the "Send Entire Balance" button\\. If you believe this is in error, try pressing generate again\\. Required (d+) WEI and got: (d+) WEI\\. [Learn More](https://myetherwallet.github.io/knowledge-base/transactions/transactions-not-showing-or-pending.html)':
    'GETH_Balance',
  ...
  '(geth-05) Intrinsic gas too low\\. Try raising the gas price to 21 GWEI via the dropdown in top-right or the gas limit to 21000 (for sending) or 200000 (for sending tokens or contracts) and try again.':
    'GETH_IntrinsicGas',
  '(geth-03) Exceeds block gas limit\\. Transaction cost exceeds current gas limit\\. Limit: (d+) WEI, got: (d+) WEI\\. Please lower the gas limit to 21000 (for sending) or 200000 (for sending tokens or contracts) and try again\\. [Learn More](https://myetherwallet.github.io/knowledge-base/gas/what-is-gas-ethereum.html)':
    'GETH_GasLimit',
...
globalFuncs.parityErrors = {
  ...
  '(parity-06) There are too many transactions in the queue\\. Your transaction was dropped due to limit\\. Try increasing the gas price\\. [Learn More](https://myetherwallet.github.io/knowledge-base/transactions/transactions-not-showing-or-pending.html)\\.':
    'PARITY_LimitReached',
  "(parity-08) TX Fee is too low\\. It does not satisfy your node's minimal fee (minimal: (d+) WEI, got: (d+) WEI)\\. Try increasing the gas price and/or gas limit\\. [Learn More](https://myetherwallet.github.io/knowledge-base/transactions/transactions-not-showing-or-pending.html)\\.":
    'PARITY_InsufficientGasPrice',
  "(parity-03) Insufficient balance\\. The account you tried to send transaction from does not have enough funds\\. If you believe this is in error, try using the 'Send Entire Balance' button, or pressing generate again. Required (d+) WEI and got: (d+) WEI\\. [Learn More](https://myetherwallet.github.io/knowledge-base/transactions/transactions-not-showing-or-pending.html)\\.":
    'ERROR_17',
  '(parity-02) Transaction cost exceeds current gas limit\\. Limit: (d+) WEI, got: (d+) WEI\\. Please lower the gas limit to 21000 (for sending) or 200000 (for sending tokens or contracts) and try again\\. [Learn More](https://myetherwallet.github.io/knowledge-base/gas/what-is-gas-ethereum.html)\\.':
...
globalFuncs.getDefaultTokensAndNetworkType = function getDefaultTokensAndNetworkType() {
  ...
  var nodeErrMsg =
    'Node does not exist, contact support@myetherwallet.com CODE:localstorageNodeMissing';
  ...
  if (isCustomNode) {
    ...
    firstCustomNodeWithMatchingNetwork = customLocalNodes.find(function(
      currentLocalCustomNode
    ) {
      return currentLocalCustomNode.options === customNodeNetworkType;
    });
    ...
    if (!firstCustomNodeWithMatchingNetwork) {
      throw Error(
        'Custom Local Node does not exist. Please clear local storage, and re-input your custom node.'
      );
    }
  } else {
    ...
    if (!defaultNode) {
      throw Error(
        'Default Node does not exist. Please clear local storage and try again.'
      );
    }
  }
  ...
  if (isCustomNode) {
    ...
    defaultTokens =
      tokenMappings[firstCustomNodeWithMatchingNetwork.options] || [];
  } else {
  ...
  return {
    ...
    networkType: isCustomNode
      ? firstCustomNodeWithMatchingNetwork.options
      : defaultNode.name.toLowerCase(),
...
function isDuplicateTokenAddress(tokenOne, tokenTwo, currentNetwork) {
  ...
  if (hasNetwork) {
    return (
      tokenTwo.network === currentNetwork &&
      tokenTwo.contractAddress === tokenOne.address
    );
  } else {
...
function isDuplicateTokenSymbol(tokenOne, tokenTwo, currentNetwork) {
  ...
  if (hasNetwork) {
    return (
      tokenTwo.network === currentNetwork && tokenTwo.symbol === tokenOne.symbol
    );
  } else {
...
globalFuncs.isDuplicateToken = function(tokenOne, tokenTwo, currentNetwork) {
  return (
    isDuplicateTokenSymbol(tokenOne, tokenTwo, currentNetwork) ||
    isDuplicateTokenAddress(tokenOne, tokenTwo, currentNetwork)
  );
};

globalFuncs.doesTokenExistInDefaultTokens = function(
  token,
  ...
  for (var i = 0; i < defaultTokensAndNetworkType.defaultTokens.length; i++) {
    ...
    var isDuplicateToken = globalFuncs.isDuplicateToken(
      currentDefaultToken,
      token,
...
globalFuncs.saveTokenToLocal = function(localToken, callback) {
  try {
    if (!ethFuncs.validateEtherAddress(localToken.contractAdd)) {
      ...
      globalFuncs.isAlphaNumericOrSpec(localToken.symbol) ||
      localToken.symbol === ''
    ) {
    ...
    for (var i = 0; i < storedTokens.length; i++) {
      if (
        storedTokens[i].symbol.toLowerCase().replace(/ /g, '') ===
        localToken.symbol.toLowerCase().replace(/ /g, '')
      ) {
        throw Error(
    ...
    for (var i = 0; i < storedTokens.length; i++) {
      if (
        storedTokens[i].contractAddress.toLowerCase().replace(/ /g, '') ===
        localToken.contractAdd.toLowerCase().replace(/ /g, '')
      ) {
        throw Error(
          'Unable to add custom token. It has the same address as custom token ' +
            storedTokens[i].symbol +
            '. Try clicking the "Load Tokens" button to see it. :)'
        );
      }
    }
